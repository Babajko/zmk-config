...
static void draw_gauge(lv_obj_t *canvas, const struct status_state *state, const struct util_position *pos) {
  lv_draw_img_dsc_t img_dsc;
  lv_draw_img_dsc_init(&img_dsc);

  const int x = pos->x;
  const int y = pos->y + 70;
  lv_canvas_draw_img(canvas, x, y, &gauge, &img_dsc);
  // lv_canvas_draw_img(canvas, 16, 43, &gauge, &img_dsc);
}

static void draw_needle(lv_obj_t *canvas, const struct status_state *state, const struct util_position *pos) {
  ...
  * int needleEndY = 66 + (int)(25.45585 * sin(4.71239)); 66 + (int)(25.45585 *
  ...
  lv_point_t points[2] = {{needleStartX + pos->x, needleStartY + pos->y},
                          {needleEndX + pos->x, needleEndY + pos->y}};
  // canvas, points, number of points, line_dsc
  lv_canvas_draw_line(canvas, points, 2, &line_dsc);
  // lv_canvas_draw_line(canvas, points, 2, &line_dsc);
}

static void draw_grid(lv_obj_t *canvas, const struct util_position *pos) {
  lv_draw_img_dsc_t img_dsc;
  lv_draw_img_dsc_init(&img_dsc);

  const int x = pos->x + -1; 
  const int y = pos->y + 95;
  lv_canvas_draw_img(canvas, x, y, &grid, &img_dsc);
  // lv_canvas_draw_img(canvas, -1, 100, &grid, &img_dsc);
}

static void draw_graph(lv_obj_t *canvas, const struct status_state *state, const struct util_position *pos) {
  lv_draw_line_dsc_t line_dsc;
...
static void draw_label(lv_obj_t *canvas, const struct status_state *state, const struct util_position *pos) {
  ...
  if (state->wpm[9] < 10) {
    const int x = pos->x + 12;
    const int y = pos->y + 75;
    lv_canvas_draw_text(canvas, x, y, 50, &label_dsc_wpm, wpm_text);
...
void draw_wpm_status(lv_obj_t *canvas, const struct status_state *state, const struct util_position *pos) {
  draw_gauge(canvas, state, pos);
  draw_needle(canvas, state, pos);
  draw_grid(canvas, pos);
  draw_graph(canvas, state, pos);
  draw_label(canvas, state, pos);
  // TODO: STAR
